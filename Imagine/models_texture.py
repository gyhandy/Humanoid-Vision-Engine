...
class Res_Double(nn.Module):
    """(convolution => [BN] => ReLU) * 2"""

    def __init__(self, in_channels, out_channels, mid_channels=None):
        super(Res_Double, self).__init__()
        if not mid_channels:
    ...
    def forward(self, x):
        return self.norm(x + self.double_conv(x))
...
class Fusion(nn.Module):
    def __init__(self, t_channel, res_depth = 9):
        ...
        # self.double_conv3 = Double(t_channel, t_channel)

        self.adain = adaptive_instance_normalization

        res_double = []
        for r in range(res_depth):
            res_double.append(Res_Double(t_channel, t_channel))

        self.res_double = nn.Sequential(*res_double)
